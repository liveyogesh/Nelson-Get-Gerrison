...
const VisitorRegistry: React.FC = () => {
  ...
  const handleCheckout = async (visitId: number) => {
    ...
    try {
      const response = await axios.post(`/api/visitors/checkout/${visitId}`);
      if (response.status === 200) fetchVisitors();
  ...
  const fetchVisitorTypes = async () => {
    try {
      ...
      setVisitorTypes(Array.isArray(data) ? data.filter((t: any) => t.active_status) : []);
    } catch (e) { console.error(e); }
  };

  const fetchEmployees = async () => {
    try {
      const { data } = await axios.get(`/api/visitors/employees`);
  ...
  const fetchDepartments = async () => {
    try {
      const { data } = await axios.get(`/api/visitors/hr/departments`);
  ...
  const fetchHistory = async (visitorId: number) => {
    try {
      const { data } = await axios.get(`/api/visitors/history/${visitorId}`);
      setVisitorHistory(Array.isArray(data) ? data : []);
    } catch (e) { console.error(e); }
  };
  ...
  const handleSubmitEntry = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const { data: result } = await axios.post(`/api/visitors/register`, {
        ...formData,
