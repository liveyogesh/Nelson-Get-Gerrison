...
export default function Reports() {
  ...
  const fetchReport = async () => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}${endpoint}?startDate=${startDate}&endDate=${endDate}&departmentId=${departmentId}`, {
        headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
      });
      const result = await response.json();
      setData(reportType === 'visitor_analytics' ? result.types || [] : result || []);
    } catch (e) {
      console.error(e);
    }
    setLoading(false);
