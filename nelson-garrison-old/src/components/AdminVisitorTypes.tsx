...
export default function AdminVisitorTypes() {
  ...
  const handleDelete = async (id: number) => {
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/visitors/types/${id}`, {
        method: 'DELETE'
      });
      if (response.ok) {
        setShowConfirm(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const url = editingType 
      ? `${import.meta.env.VITE_API_URL}/api/visitors/types/${editingType.type_id}`
      : `${import.meta.env.VITE_API_URL}/api/visitors/types`;
    ...
    try {
      ...
      if (response.ok) {
        setShowModal(false);
        setEditingType(null);
        setFormData({ type_name: '', description: '', active_status: true });
        fetchTypes();
      }
    } catch (e) { console.error(e); }
  };
