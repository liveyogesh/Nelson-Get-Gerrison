...
router.get('/visitor-analytics', async (req: any, res: any) => {
  try {
    ...
    let freqQuery = `SELECT DATE(check_in_time) as date, COUNT(*) as count FROM visitor_visits WHERE 1=1 AND check_in_time IS NOT NULL`;
    let freqParams: any[] = [];
    if (!req.isCorporate) {
      if (req.facilities && req.facilities.length > 0) {
    ...
    freqQuery += ` GROUP BY DATE(check_in_time) ORDER BY date DESC LIMIT 30`;
    ...
    res.status(500).json({ error: 'Failed to fetch report data', msg: error.message });
  }
});
