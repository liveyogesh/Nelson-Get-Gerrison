  ...
  try {
    let query = `
      SELECT l.*, z.zone_name, z.zone_code,
        CASE
           WHEN l.person_type = 'EMPLOYEE' THEN (SELECT CONCAT(first_name, ' ', COALESCE(last_name, '')) FROM hr_employees WHERE employee_id = l.person_reference_id)
           WHEN l.person_type = 'VISITOR' THEN (SELECT CONCAT(first_name, ' ', COALESCE(last_name, '')) FROM visitor_master WHERE visitor_id = l.person_reference_id)
        END as person_name,
