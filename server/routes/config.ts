...
router.get('/facilities', async (req: any, res: any) => {
  try {
    const [rows]: any = await db.query('SELECT facility_id, facility_name, facility_code FROM org_facilities WHERE operational_status = "ACTIVE"');
    res.json(rows);
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
